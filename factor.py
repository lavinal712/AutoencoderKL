...
if __name__ == "__main__":
    ...
    dataset = ImageNetDataset(opt.datadir, split="train", transform=transform)
    ...
    sum_z = torch.tensor(0.0, device=device)
    sum_z2 = torch.tensor(0.0, device=device)
    num_items = torch.tensor(0, device=device, dtype=torch.long)
    ...
    dist.all_reduce(sum_z2, op=dist.ReduceOp.SUM)
    dist.all_reduce(num_items, op=dist.ReduceOp.SUM)

    if rank == 0:
        mean = sum_z / num_items
        var = sum_z2 / num_items - mean ** 2
        std = torch.sqrt(var)

        print(f"Scale factor: {1 / std.item():.6f}")
        print(f"Shift factor: {mean.item():.6f}")
